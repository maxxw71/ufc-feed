...
def card_html(card):
    ...
    for m in methods:
        ...
        checks=[_check_text(x).strip() for x in (m.get('checks') or []) if _check_text(x).strip()]
        block=''
    ...
    details=''
    if detail_parts:details='<details class="details-block"><summary>Method details & qualification checks</summary><div class="details-inner">'+''.join(detail_parts)+'</div></details>'
...
def safe_enhance(sp):
    try:
        ...
        print('UFC_PUBLIC_PAGE',{'upcoming':len(upcoming),'completed':sum(dt(c['start'])<=now for c in cards),'nfl_layout':True,'compact_cards':True})
