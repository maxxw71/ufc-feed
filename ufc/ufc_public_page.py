...
def method_key():
    methods=[
        ('U1','Age Hybrid','Market-supported favorite with a meaningful age advantage; stronger tier requires a larger age edge.'),
        ('U2','Reach Hybrid','Market-supported favorite with a clear reach advantage; stronger tier uses the larger reach threshold.'),
        ('U3','Structural Edge','Combines market support, a younger favorite and a large reach advantage, with the skill veto clear.'),
...
def safe_enhance(sp):
    try:
        ...
        page=f'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="Appwiza UFC method watchlist and tracked results."><title>UFC watchlist | Appwiza</title><link rel="stylesheet" href="/sports/style.css"><link rel="stylesheet" href="/sports/ufc/dashboard.css?v={now.astimezone(NY).strftime("%Y%m%d%H%M")}"></head><body>{nav}<main><span class="eyebrow">Appwiza · UFC</span><h1>UFC qualifying matchups</h1><p class="sub">Method-qualified selections used by the UFC scanner, grouped by event date, with recorded prices and historical evidence.</p><div class="bar"><span>Updated: {esc(updated)}</span><span>{len(upcoming)} upcoming · nearest events first</span></div>{record(cards,upcoming)}{research}{audit_section(upcoming)}<div class="section-kicker">Current board</div><h2>Available selections</h2>{group_upcoming(upcoming)}<div class="section-kicker">Tracked results</div><h2>Completed selections</h2><p class="muted">One unit per tracked selection at the recorded moneyline. Pending results are excluded from ROI.</p>{results(cards)}{withdrawn_html}{method_key()}{downloads(sp)}<footer>Historical method performance does not guarantee future results. Quotes can move; confirm the current sportsbook price before acting.</footer></main></body></html>'
        atomic(PUBLIC/'dashboard.css',DASH_CSS);atomic(PUBLIC/'index.html',page);print('UFC_PUBLIC_PAGE',{'upcoming':len(upcoming),'completed':sum(dt(c['start'])<=now for c in cards),'nfl_hierarchy':True})
