...
# reconstruct H sets with non-coaching hard vetoes only (same independence standard as deep dissection)
hsets={}
...
for cm,hm in pairs:
    ...
    # filtered version using final selected CO veto, if present
    versions=[('RAW',z)]
    sv=sel[sel.method_id.eq(cm)] if len(sel) else pd.DataFrame()
    if len(sv):
        q=sv.iloc[0];zf=z[~cmask(z,q.feature,q.op,float(q.threshold))].copy();versions.append(('CO_VETO',zf))
    for version,x in versions:
        ...
        for _,r0 in x[x.win.eq(0)].iterrows():
            losses.append({'pair':cm+'+'+hm,'version':version,'season':int(r0.season),'week':int(r0.week),'game_id':r0.game_id,'team':r0.team,'opponent':r0.opponent,'moneyline':r0.moneyline})
...
for r in rows:
    lines.append(f"{r['pair']} {r['version']} | {r['wins']}-{r['losses']} ({100*r['win_pct']:.1f}%) n={r['n']} ROI={100*r['roi']:+.1f}% | positive seasons={r['positive_seasons']}/{r['active_seasons']} ({100*r['positive_season_ratio']:.0f}%) max season share={100*r['max_season_share']:.1f}% | LOO min ROI={100*r['loo_min_roi']:+.1f}% all profitable={r['loo_all_profitable']}")
