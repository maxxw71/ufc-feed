...
r=requests.get(URL,headers={'User-Agent':'Mozilla/5.0 (compatible; AppWizaNFLResearch/1.0)'},timeout=40)
r.raise_for_status()
text=BeautifulSoup(r.text,'html.parser').get_text(' ',strip=True)
# Slice by full team name. Within each team block the public page labels coordinators explicitly.
positions=[]
...
for idx,(start,full,code) in enumerate(positions):
    ...
    # Keep the shortest plausible trailing name if regex swallowed prior labels.
    def tail(m):
...
if len(valid)<28:
    raise RuntimeError(f'Only parsed {len(valid)} complete 2026 staffs from ChatFFB')
...
for code,v in valid.items():
    ...
    staff.loc[mask,'staff_source']='chatffb_2026_official_team_site_rollup'
...
status['chatffb_2026']={'ok':True,'teams_parsed':len(valid),'url':URL}
status['oc_filled']=int(staff.offensive_coordinator.notna().sum())
status['dc_filled']=int(staff.defensive_coordinator.notna().sum())
