...
# Promote robust flagship families. Keep H001 in review because its extreme long-dog ROI warrants extra forensic review.
...
# Fail-safe: anything with weak season consistency stays watchlist instead of final.
...
summary={'flagship_methods':int(len(m)),'final_research':int((m.registry_status=='FINAL_RESEARCH').sum()),'watchlist':int((m.registry_status=='WATCHLIST').sum()),'review_extreme_roi':int((m.registry_status=='REVIEW_EXTREME_ROI').sum()),'methods_with_validated_vetoes':int((m.validated_vetoes!='').sum())}
