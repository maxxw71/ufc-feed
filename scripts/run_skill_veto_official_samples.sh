...
age_path,reach_path,stats_path,outdir=map(Path,sys.argv[1:5])
...
def boolify(s):
    ...
    return s.astype(str).str.strip().str.lower().map({'true':True,'1':True,'yes':True,'win':True,'w':True,'false':False,'0':False,'no':False,'loss':False,'l':False}).fillna(False).astype(bool)
...
def profit_series(df, won):
    c=first_col(df,['profit_100','profit','pl_100','pnl_100','bet_profit_100','flat_profit_100'],False)
    if c:
        return pd.to_numeric(df[c],errors='coerce').fillna(0.0)
    c=first_col(df,['american_odds','best_american_odds','favorite_odds','fav_odds','odds'],False)
    if c:
    ...
    c=first_col(df,['decimal_odds','best_decimal','favorite_decimal','fav_decimal'],False)
    if c:
...
def metric(g):
    n=len(g)
    if not n:return dict(n=0,w=0,l=0,roi=np.nan,p=0)
    w=int(g['_won'].sum()); p=float(g['_profit100'].sum())
...
stats['favn']=stats['favorite'].map(norm); stats['dogn']=stats['underdog'].map(norm)
...
def merge_skill(df,date_col,fav_col,dog_col):
    ...
    z['favn']=z[fav_col].map(norm); z['dogn']=z[dog_col].map(norm)
...
def finish_method(name,df):
    ...
    k=metric(df.loc[~veto]); v=metric(df.loc[veto])
    print(name)
    print('-'*108)
    ...
    print()
    return {'method':name,'baseline_bets':b['n'],'baseline_wins':b['w'],'baseline_losses':b['l'],'baseline_roi':b['roi'],'baseline_profit':b['p'],'filtered_bets':k['n'],'filtered_wins':k['w'],'filtered_losses':k['l'],'filtered_roi':k['roi'],'filtered_profit':k['p'],'roi_change_pp':(k['roi']-b['roi'])*100,'removed_bets':v['n'],'removed_wins':v['w'],'removed_losses':v['l'],'removed_roi':v['roi']}
...
if len(age)!=889: print(f'WARNING: Age master has {len(age)} rows; expected 889.')
summary.append(finish_method('AGE HYBRID — OFFICIAL MASTER',age))

# REACH and PREMIUM official samples
reach=pd.read_csv(reach_path,low_memory=False)
for c in ['market_prob','fav_reach_adv','fav_younger_by','profit_100']:
    if c in reach.columns: reach[c]=pd.to_numeric(reach[c],errors='coerce')
rdate=first_col(reach,['event_date','date','fight_date'])
rfav=first_col(reach,['favorite','fav_name','favorite_name','market_favorite'])
rdog=first_col(reach,['underdog','dog_name','underdog_name','market_underdog'])
rwc=first_col(reach,['favorite_won','won','fav_won'],False)
if rwc: reach['_won']=boolify(reach[rwc])
else:
    rc=first_col(reach,['result','outcome'])
    reach['_won']=boolify(reach[rc])
...
if len(reach_off)!=415: print(f'WARNING: Reach official filter produced {len(reach_off)} rows; expected 415.')
summary.append(finish_method('REACH HYBRID — OFFICIAL 415',reach_off))

premium=reach[(reach.market_prob>=.70)&(reach.fav_younger_by>=3)&(reach.fav_reach_adv>=4)].copy()
premium=merge_skill(premium,rdate,rfav,rdog)
if len(premium)!=133: print(f'WARNING: Premium official filter produced {len(premium)} rows; expected 133.')
