...
# These pages were explicitly observed in public search/site navigation and are
# used only as discovery pages for links they themselves publish.  They are not
# generated by numeric enumeration.  This substantially improves 2025-2026
# round coverage while preserving a conservative provenance trail.
DISCOVERY_PAGES=[
...
def discover_from(page,links,report_pages):
    try:
        soup=soup_for(page)
        links.update(urljoin(page,a['href']) for a in soup.find_all('a',href=True) if 'round-stats/' in a['href'])
        report_pages.update(urljoin(page,a['href']) for a in soup.find_all('a',href=True) if '/reports/' in a['href'])
...
links=[url for url in sorted(links) if not db.execute("SELECT 1 FROM punch_reports WHERE url=? AND status='parsed'",(url,)).fetchone()][:240]
...
for url in links:
    try:
        ...
        if cached and (ROOT/cached[0]).exists():raw=(ROOT/cached[0]).read_bytes()
            ...
            time.sleep(1)
            with urlopen(Request(url,headers={'User-Agent':'BoxingHistoryResearch/1.2','Accept-Encoding':'identity'}),timeout=30) as response:raw=response.read(8_000_001)
