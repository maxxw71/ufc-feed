...
No fuzzy guessing and no external profile backfill. A new link is accepted only
when exactly one candidate fighter identity has a same-date reciprocal bout,
the opponent names point back to the source fighter through observed aliases,
and the outcomes are mutually consistent.
"""
...
def discover(rows,fighter_names,existing):
    ...
    additions={}
    ambiguous=0; no_candidate=0; no_recip=0
    for r in rows:
        bid=r['source_id']
...
def main():
    ...
    rows=[dict(r) for r in con.execute("select * from bouts where source='wikipedia' and status='FINISHED' and date is not null order by date,source_id")]
    names={r['source_id']:r['name'] for r in con.execute("select source_id,name from fighters where source='wikipedia'")}
    ...
    con.commit()
    report={'existing_verified_links':len(existing),'new_verified_alias_links':len(additions),
            'combined_links':len(existing)+len(additions),'diagnostics':diag,
    ...
    (ROOT/'IDENTITY_LINKS_V2.json').write_text(json.dumps(report,indent=2))
