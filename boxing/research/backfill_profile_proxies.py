...
def parse_tapology(name):
    ...
    for a in soup.find_all('a',href=True):
        ...
        if url not in links:
            links.append(url)
    if not links:
        raise ValueError('Tapology search returned no fighter links')
    errors=[]
    for url in links[:8]:
        try:
            ...
            page_name=labeled(lines,'Name')
            if not page_name:
                h=s.find('h1')
                page_name=clean(h.get_text(' ',strip=True)) if h else ''
            if nk(page_name)!=nk(name):
                exact=next((clean(x.split(':',1)[1]) for x in lines
                            if x.casefold().startswith('name:')
                            and nk(clean(x.split(':',1)[1]))==nk(name)),None)
                if not exact:
                    continue
            data={}
            hv=cm(labeled(lines,'Height'))
            rv=cm(labeled(lines,'Reach'))
            if hv is not None and 120<=hv<=250:
                data['height_cm']=hv
            if rv is not None and 120<=rv<=270:
                data['reach_cm']=rv
            sv=stance(labeled(lines,'Stance'))
            if sv:
                data['stance']=sv
            if not data:
                raise ValueError('Tapology exact page had no usable profile fields')
