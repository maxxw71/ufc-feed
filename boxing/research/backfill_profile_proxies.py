...
def parse_boxingscene(name):
    ...
    h=soup.find('h1')
    heading=clean(h.get_text(' ',strip=True)) if h else ''
    heading=re.sub(r'\s+(?:Record|Profile(?:\s*&\s*Stats)?)\s*
...
def main():
    ...
    for x in candidates[:args.limit]:
        ...
        # BoxRec wiki currently rejects the GitHub runner with 403s, so use
        # BoxingScene plus Tapology for automated public-profile enrichment.
        # The BoxRec parser remains available for environments with authorized
        # access but is not hammered on every batch.
        for parser in (parse_boxingscene,parse_tapology):
            try:
                e=parser(name)
...
def parse_tapology(name):
    search='https://www.tapology.com/search?term='+urllib.parse.quote_plus(name)
    _,raw=fetch(search)
    soup=BeautifulSoup(raw,'lxml')
    links=[]
    for a in soup.find_all('a',href=True):
        href=a.get('href') or ''
        if '/fightcenter/fighters/' not in href:continue
        url=urllib.parse.urljoin(search,href)
        if url not in links:links.append(url)
    if not links:raise ValueError('Tapology search returned no fighter links')
    errors=[]
    for url in links[:8]:
        try:
            ...
            page_name=labeled(lines,'Name') or labeled(lines,'Given Name')
            if not page_name:
                h=s.find('h1');page_name=clean(h.get_text(' ',strip=True)) if h else ''
            # "Given Name" may be fuller than ring identity, so only use it when
            # the page's explicit Name label is absent and normalization matches.
            if nk(page_name)!=nk(name):
                # Search result anchors often omit/append nicknames. Look for an
                # exact explicit "Name: X" line before rejecting.
                exact=next((clean(x.split(':',1)[1]) for x in lines
                            if x.casefold().startswith('name:') and nk(clean(x.split(':',1)[1]))==nk(name)),None)
                if not exact:continue
            data={}
            hv=cm(labeled(lines,'Height'));rv=cm(labeled(lines,'Reach'))
            if hv is not None and 120<=hv<=250:data['height_cm']=hv
            if rv is not None and 120<=rv<=270:data['reach_cm']=rv
            bv=iso_date(labeled(lines,'Age') or labeled(lines,'Date of Birth'))
            if bv:data['born']=bv
            born=labeled(lines,'Born')
            # Tapology's Born field is commonly birthplace text, not nationality.
            sv=stance(labeled(lines,'Stance'))
            if sv:data['stance']=sv
            if not data:raise ValueError('Tapology exact page had no usable profile fields')
            return {'source':'tapology','url':final,'fields':data}
        except Exception as e:
            errors.append(str(e)[:120])
    raise ValueError('Tapology exact identity not resolved: '+('; '.join(errors[:3]) if errors else 'no exact page'))
