...
UA='Mozilla/5.0 AppwizaBoxingCareerBackfill/1.1'
...
def direct_titles(name):
    ...
    # Common suffix normalization used by Wikipedia titles.
...
def search_titles(name):
    titles=[]
    # Direct candidates are important when the MediaWiki search endpoint is
    # rate-limited or when a lesser-known fighter is not returned near the top.
    titles.extend(direct_titles(name))
...
def discover_from_opponents(item,cache):
    """Find target page links from exact dated rows on known opponent pages."""
    found=[]
    target_key=nk(item['name'])
    for e in item['evidence'][:6]:
        for opp_title in search_titles(e['opponent'])[:8]:
            try:
                page=cache.setdefault(opp_title,parse_page(opp_title))
            except Exception:
                continue
            for r in page['rows']:
